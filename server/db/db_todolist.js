...
async function connectDB(){
    try{
        await client.connect(); //몽3. 몽고 접속
          ...
          db_project = client.db('project');//몽4. ***프로젝트db 활성화***(제일 중요)
          console.log('접속완료') 
    }
    catch(err){
        console.log(err)
...
function getContactDB(){//connectDB가 비동기라 값이 언제 들어올지 몰라서 사용하려는 시점에 db에 값이 들어있으려면 
    //connectDB가 끝난후에 getDB가 실행할것이므로 그때 db값 가져오기
    return db_project;
}
module.exports={connectDB, getDB, getContactDB}